...
public class TesterNital {
    ...
    public static boolean isNital(int[][] square) {
        ...
        //checks if columns have same values
        int[] fcol = getColumn(square, 0);
        for (int i = 1; i < square.length; i++) {
            int[] ncol = getColumn(square, i);
            //not same numbers
            if (!(hasAllValues(fcol, ncol))) {
                return false;
            }
        }


        //checks if columns don't repeat
        for (int i = 0; i < square[0].length; i++) {
            int[] a = getColumn(square, i);
            if (containsRepeats(a)) {
                return false;
            }
        }
        return true;
